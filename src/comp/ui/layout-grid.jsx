    ...
    <div className="grid w-full h-full grid-cols-1 gap-4 p-10 mx-auto md:grid-cols-3 max-w-7xl ">
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className, "")}>
          ...
          >
            {selected?.id === card.id && (
              <SelectedCard setSelected={setSelected} selected={selected} />
            )}
            <BlurImage card={card} />
          </motion.div>
        </div>
...
const SelectedCard = ({ selected, setSelected }) => {
  return (
    <div className="bg-transparent h-full w-full flex flex-col justify-center rounded-lg shadow-2xl absolute z-[60]">
      <motion.div
